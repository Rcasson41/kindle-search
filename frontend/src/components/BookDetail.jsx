  ...
  return (
    <div className="flex gap-1 items-center">
      {[1, 2, 3, 4, 5].map((n) => (
        <svg
          key={n}
          className={`w-5 h-5 ${n <= rating ? "text-yellow-400" : "text-gray-700"}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
  ...
  async function handleShelfChange(e) {
    ...
    try {
      const res = await fetch(`/books/${id}`, {
        ...
        body: JSON.stringify({ shelf: shelf || null }),
      });
        ...
        <div className="flex flex-col sm:flex-row gap-8">
          ...
          <div className="flex flex-col gap-4 flex-1">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {book.shelf && (
                <span className={`text-sm px-3 py-1 rounded-full font-medium ${SHELF_STYLES[book.shelf] || "bg-gray-800 text-gray-400"}`}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                disabled={saving}
                className="w-fit bg-gray-800 border border-gray-700 text-gray-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500 disabled:opacity-50"
              >
                {SHELF_OPTIONS.map((o) => (
            ...
            {genres.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {genres.map((g) => (
                  <span key={g} className="text-xs bg-gray-800 text-gray-400 px-2 py-1 rounded">
                    {g}
                  </span>
                ))}
              </div>
            ...
            {book.page_count && (
              <p className="text-xs text-gray-500">{book.page_count} pages{book.published_date ? ` · ${book.published_date}` : ""}{book.publisher ? ` · ${book.publisher}` : ""}</p>
            )}
